...
# Pin Definitions
output_red = 12  # BOARD pin 12
output_yellow = 13  # BOARD pin 13
output_green = 15  # BOARD pin 15
input_kill = 37 #Board pin 37
# Pin Setup
GPIO.setmode(GPIO.BOARD)
GPIO.setup(output_red, GPIO.OUT, initial=GPIO.LOW)# Red = Emergency switch pressed
GPIO.setup(output_yellow, GPIO.OUT, initial=GPIO.HIGH)# Yellow = Manual mode
GPIO.setup(output_green, GPIO.OUT, initial=GPIO.LOW)# Green = Autonomous mode
GPIO.setup(input_kill, GPIO.IN) # Read when killswitch is activated
...
WP_lon = open(r"WP_lon_turn.txt","w")
WP_lat = open(r"WP_lat_turn.txt","w")
WP_x = open(r"WP_xValueturn.txt","w")
WP_y = open(r"WP_yValueturn.txt","w")
WP_launch_lon = open(r"WP_launch_lonturn.txt","w")
WP_launch_lat = open(r"WP_launch_latturn.txt","w")
#Global variables
check = None
glob = ''
rc_input = None
...
def change_mode(mode):
	...
	if mode == 'AUTO' and input_kill==GPIO.HIGH:
		GPIO.output(output_red,GPIO.LOW)
		GPIO.output(output_yellow,GPIO.LOW)
		GPIO.output(output_green,GPIO.HIGH)
	if mode == 'MANUAL' and input_kill==GPIO.HIGH:
		GPIO.output(output_red, GPIO.LOW)
		GPIO.output(output_yellow, GPIO.HIGH)
		GPIO.output(output_green, GPIO.LOW)	
...
def create_waypoint(x,y):
	...
	wp2.param3 = 5  # Pass Radius. If 0 go through WP
	...
	wp2 = Waypoint()   #Creates new instance of WayPoint
	wp2.frame = 0 # PX4 Only supports Global frame
	wp2.command = 19  #HOLD position
	wp2.is_current = False
	wp2.autocontinue = True
	wp2.param1 = 30  # HOLD time at WP
	wp2.param2 = 0  # Acceptance radius, if inside WP count as reached
	...
	wp2.param3 = 0  # Pass Radius. If 0 go through WP
	wp2.param4 = 0 #float('nan')  # Yaw, 0 for our USV situation
	wp2.x_lat = 0 #movement North/South
	wp2.y_long = 0 #movement East/West
	wp2.z_alt = 0
	wl.append(wp2)
...
def set_wp(orientation):
	...
	if not check:
		key = glob
		x = 2 * dict_to_float(key,dict_x)
		y = 2 * dict_to_float(key,dict_y)
		x_lat = latitude(x)
		y_lon = longitude(y)
		create_waypoint(x_lat,y_lon)
		WP_x.write(str(x_lat))	
		WP_x.write("\n")
		WP_y.write(str(y_lon))
		WP_y.write("\n")
		check = True
###Translate desired distance into latitude
def latitude(meters):
	earth = 6371000
	m = (1 / ((2 * pi / 360) * earth))
	new_latitude = lat + (meters * m)
	return new_latitude
###Translate desired distance into longitude
...
while not rospy.is_shutdown():
	killswitch = GPIO.input(input_kill)
	sub_gps
	sub_compass
	if rc_input == True:
		rospy.sleep(1)
	...
	if killswitch == GPIO.LOW:
            GPIO.output(output_red, GPIO.HIGH)
            GPIO.output(output_yellow, GPIO.LOW)
            GPIO.output(output_green, GPIO.LOW)
	rospy.sleep(1)
